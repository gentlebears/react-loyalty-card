						...
						<div className="flex flex-col sm:flex-col md:flex-row lg:flex-row rounded-3xl shadow-lg py-5 lg:py-0 md:py-0 sm:py-0 ">
							...
							<div className="px-5 h-full my-auto lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center">
								...
								<p className="mb-4 lg:mb-2 leading-relaxed">
									{brands[useId].name} ile yaptığınız
									alışverişlerden {brands[useId].totalGoal}.
									kahve bizden hediye! {appname} aracılığı ile
									aldınız her kahve size ekstra puan
									kazandırır, aldığınız kahveleri sistemimiz
									otomatik olarak sayar ve{" "}
									{brands[useId].totalGoal}. kahveye
									geldiğinizde hiç bir ödeme yapmanıza gerek
								...
								<div className="flex justify-center gap-5 sm:mb-3">
									<Link
										to={"/detail/" + brands[useId].name}
										className="flex"
									>
										<button
											onClick={() => {
												brands[useId].subscribe =
													!brands[useId].subscribe;
											}}
